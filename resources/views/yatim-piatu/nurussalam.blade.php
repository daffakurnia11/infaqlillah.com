...
<h6 class="mb-0 text-uppercase d-flex justify-content-between align-items-center">
  <span>Data Yayasan Nurussalam</span>
  <a href="/admin/yatim-piatu/create" class="btn btn-sm btn-primary">Tambah Pengeluaran</a>
</h6>
<hr/>
<div class="card">
  <div class="card-body">
        ...
        <tbody>
          @foreach ($datas as $data)
            <tr>
              ...
              <td class="align-middle text-nowrap">{{ $data->receiver }}</td>
              <td class="align-middle text-center text-nowrap">Rp {{ $data->nominal }},00</td>
              <td class="align-middle text-center text-wrap">{{ $data->period }}</td>
              <td class="align-middle text-center text-nowrap">
                @if ($data->photo)
                <a href="/img/foto_yayasan/{{ $data->photo }}" target="_blank"><i class="bi bi-eye-fill"></i> Lihat Foto</a>
                @else
                <i>Tidak ada foto</i>
                @endif
              </td>
              <td class="align-middle text-nowrap">
                <div class="table-actions d-flex align-items-center justify-content-center gap-3 fs-6">
                  ...
                  <a href="/admin/yatim-piatu/{{ $data->id }}/edit" class="text-warning" data-bs-toggle="tooltip" data-bs-placement="bottom" title="" data-bs-original-title="Ubah" aria-label="Ubah"><i class="bi bi-pencil-fill"></i></a>
                  <form action="/admin/yatim-piatu/{{ $data->id }}" method="POST">
                    @csrf
                    @method('DELETE')
